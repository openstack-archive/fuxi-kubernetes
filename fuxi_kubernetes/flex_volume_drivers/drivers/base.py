...
class BaseVolumeDriver(object):
        ...
        def _send_and_receive():
            try:
                url = 'http://%(ip)s:%(port)d%(api)s' % (
                    {'ip': self._driver_service_ip,
                     'port': self._driver_service_port,
                     'api': api})
                ...
                return (False,
                        {'message': 'Some exception:(%s) happened '
                                    'during request for server' % str(ex)})
    ...
    def __call__(self, argv):
        ...
        self._driver_service_ip = argv[0]
        self._driver_service_port = int(argv[1])
        argv = argv[3:]
