...
class DeltaFIFO(object):
    ...
    stores a series of events of same resource. The event is described by
    structure of 'Delta'.

    'self._queue' is a list and stores the keys of 'self._items'.

    The behavior of FIFO is implemented like this. Suppose the are two
    ...
    Pop event from DeltaFIFO like this. Pop the frist item of self._queue
    and pop the corresponding event in self._items[key] from left to right,
    then loop again untile self._queue is empty. For the example above, the
    events popped will be EA1 EA2 EB1 and EB2.
    """
