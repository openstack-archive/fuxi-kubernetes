...
class Producer(object):
    ...
    def _list(self):
        try:
            ...
            LOG.error("Producer list resource failed, reason:%s", str(ex))
            return None, ex
    ...
    def _watch(self, resource_version):
        try:
            ...
            LOG.error('Producer watch resource failed, reason:%s', str(ex))
            return None, ex
        ...
        try:
            ...
            for e in events:
                ...
                if event_type == constants.WATCH_EVENT_TYPE_ERROR:
                    LOG.error("Producer encounter an event of ERROR")
            ...
            LOG.error('Producer handle event failed, reason:%s', str(ex))
            return None, ex
